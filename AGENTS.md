...
所有提交信息统一使用中文说明。提交标题使用 `[类型] 一句话说明行为变化` 格式，类型使用英文小写标签，建议从 `[feat]`、`[fix]`、`[refactor]`、`[test]`、`[docs]`、`[build]` 中选择，例如 `[feat] 增加模型请求重试`。一句话应说明具体行为变化，避免笼统的 `更新`、`修改`、`调整`。PR 应包含变更摘要、验证命令与结果、关联 issue；涉及 CLI 输出或配置流程时，附上示例命令或截图。不要在 PR 中包含本地密钥、真实 `model.properties` 或生成产物。
